...
static void *threadpool_thread(void *threadpool) {
    ...
    while (true) {
        ...
        if (pool->shutdown) {
            /*TODO: sync?*/
            if (pool->sync && !ringbuffer_is_empty(pool->task_list))
            {
                threadpool_task_t new_task;
                bool success = ringbuffer_pop(pool->task_list, &new_task);
                lock_release(pool);
                if (success)
                {
        ...
        bool success = ringbuffer_pop(pool->task_list, &task);
        ...
        /* 7. Get to work */
        // TODO: run next task
        if (success) /*we have fetched the task*/
        {
            (*(task.func))(task.args);
        }
    }
...
bool threadpool_add_task(threadpool_t *pool, void (*func)(void *), void *args) {
    ...
    new_task.args = args;
    new_task.func = func;
    if (!ringbuffer_push(pool->task_list, new_task)) /*this is not a successful push*/
    {
        lock_release(pool);
        return false;
    }
